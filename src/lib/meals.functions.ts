...
const SYSTEM_PROMPT = `You are a nutrition estimation engine. Estimate macros for a described meal as accurately as possible.
...
export const estimateMeal = createServerFn({ method: "POST" })
  ...
  .inputValidator((data: EstimateInput) => {
    ...
    if (data.portionNotes && data.portionNotes.length > 1000) throw new Error("Portion notes too long");
    const hint =
  ...
  .handler(async ({ data }) => {
    ...
    const userParts = [`Meal: ${data.description}`];
    if (data.portionNotes) userParts.push(`Portion notes: ${data.portionNotes}`);
    if (data.userKcalHint != null)
      userParts.push(
        `User's own kcal estimate (calibrate to within ~10%): ${data.userKcalHint} kcal`,
      );
